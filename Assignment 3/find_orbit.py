...
def propagate_orbit_eci(
    ...
    return np.vstack((sol.y[0], sol.y[1], sol.y[2]))
...
def run_all_gsat() -> Tuple[np.ndarray, np.ndarray]:
    ...
    tle_file = "Assignment 3/TLE.txt"  # path to your single TLE file
    mode = 1 # which simulator you want
    ...
    orbit_eci = propagate_orbit_eci(
        e, i_deg, raan_deg, argp_deg, h, mu, theta0_deg, t_eval, mode
    )
    
    # --- Convert to ECEF and Lat/Lon ---
    orbit_ecef = transform.ECI_to_ECEF(orbit_eci, t_eval, 7.2921159e-5)
    latlon_full = eci_to_latlon_2xN(orbit_eci, t_eval)

    # --- Plot results ---
    plot_multiple_orbits_eci([orbit_eci], ["Buccaneer"])
